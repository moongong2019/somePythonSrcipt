...
def get_logits(input_frames, num_actions,keep_prob):
    #keep_prob = tf.placeholder(tf.float32)
    conv1_W = tf.get_variable(shape=[6, 6, 4, 64], name='conv1_W',
        initializer=tf.contrib.layers.xavier_initializer())
    conv1_b = tf.Variable(tf.zeros([64], dtype=tf.float32),
        name='conv1_b')
    conv1 = tf.nn.conv2d(input_frames, conv1_W, strides=[1, 3, 3, 1],
    ...
    conv2_W = tf.get_variable(shape=[4, 4, 64, 64], name='conv2_W',
         initializer=tf.contrib.layers.xavier_initializer())
    ...
    fc2_W = tf.get_variable(shape=[512, num_actions], name='fc2_W',
         initializer=tf.contrib.layers.xavier_initializer())
